...
`,d+="🏆 *Selected Sports Auction Platform*",d}function Mr(e,t){const n=or(e,t);if(!n)return;const a=`https://api.whatsapp.com/send?text=${encodeURIComponent(n)}`;window.open(a,"_blank")}function Wr(e,t){if(!t||t.length===0){alert("No players found in the auction pool to export.");return}const n=m=>String(m??"").replace(/[&<>"']/g,h=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[h]),a=new Date().toLocaleDateString("en-IN",{day:"numeric",month:"long",year:"numeric"}),r=(e==null?void 0:e.name)||"Selected Sports Cricket Auction",s=t.reduce((m,h)=>m+(Number(h.base_price)||0),0);let l=0,d=0,c=0,u=0;t.forEach(m=>{const h=(m.playing_role||"").toLowerCase();h.includes("all")?l++:h.includes("bat")?d++:h.includes("bowl")?c++:(h.includes("keep")||h.includes("wk"))&&u++});const p=t.map((m,h)=>{const w=h+1,k=m.playing_role||"Player",_=k.toLowerCase();let b="role-other",v="🏏";_.includes("all")?(b="role-all",v="🏏"):_.includes("bat")?(b="role-bat",v="⚡"):_.includes("bowl")?(b="role-bowl",v="🎯"):(_.includes("keep")||_.includes("wk"))&&(b="role-keep",v="🧤");const z=(m.name||"?").slice(0,1).toUpperCase(),P=m.profile_image_url?`<img src="${n(m.profile_image_url)}" alt="${n(m.name)}" class="player-photo" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';" /><div class="player-initials-fallback" style="display:none;">${n(z)}</div>`:`<div class="player-initials-fallback">${n(z)}</div>`;return`
      <div class="player-card">
        <div class="card-top">
          <span class="lot-badge">#${w<10?"0"+w:w}</span>
          <span class="role-badge ${b}">${v} ${n(k)}</span>
        </div>
