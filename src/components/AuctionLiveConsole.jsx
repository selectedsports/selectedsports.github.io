  ...
  if (!state || state.status === "setup") {
    return (
      <Card style={{ padding:"24px 20px" }}>
        <div style={{ fontWeight:800, fontSize:16, color:"#0F172A", fontFamily:"var(--font-head)", marginBottom:6 }}>Start the Live Auction</div>
        <div style={{ fontSize:13, color:"#64748B", marginBottom:16 }}>{auctionPlayers.filter(p=>p.status==="registered").length} players in the pool · {auctionTeams.length} teams ready</div>
        ...
        <button onClick={doStart} disabled={busy || auctionTeams.length < 2} style={{ width:"100%", padding:"14px", borderRadius:10, background:"#166534", border:"none", color:"#FFFFFF", fontSize:14, fontWeight:800, cursor:busy||auctionTeams.length<2?"not-allowed":"pointer", opacity:busy||auctionTeams.length<2?0.5:1, fontFamily:"var(--font-head)" }}>{busy ? "Starting..." : "Start Auction"}</button>
      </Card>
    )
  }
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, color:"#94A3B8", marginBottom:14 }}>
        <span>{registeredCount} left in pool</span><span>{soldCount} sold · {unsoldCount} unsold</span>
      </div>
      ...
      {!currentPlayer ? (
        ...
        <Card style={{ padding:"18px 16px", marginBottom:14, border:"2px solid #166534" }}>
          <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:14 }}>
            <Av name={currentPlayer.name} id={currentPlayer.id} sz={44}/>
          ...
          <div style={{ textAlign:"center", padding:"14px", background:"rgba(34,197,94,0.08)", borderRadius:12, marginBottom:14 }}>
            <div style={{ fontSize:28, fontWeight:900, color:"#166534", fontFamily:"var(--font-head)" }}>₹{state.current_bid}</div>
            <div style={{ fontSize:12, color:leadingTeam?"#166534":"#94A3B8", fontWeight:700, marginTop:2 }}>{leadingTeam ? `Leading: ${leadingTeam.name}` : "No bids yet"}</div>
          </div>
            ...
            {auctionTeams.map(t => {
              ...
              return (
                <button key={t.id} onClick={()=>doBid(t.id)} disabled={busy || !canAfford || isLeading} style={{ padding:"10px 8px", borderRadius:10, border:isLeading?"2px solid #166534":"1.5px solid #E2E8F0", background:isLeading?"rgba(34,197,94,0.1)":"#FFFFFF", cursor:(busy||!canAfford||isLeading)?"not-allowed":"pointer", opacity:(!canAfford||isLeading)?0.5:1, textAlign:"center" }}>
                  ...
                  <div style={{ fontSize:10, color:"#94A3B8" }}>₹{t.purse_remaining} left</div>
                </button>
              )
            })}
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8 }}>
            <button onClick={doUndo} disabled={busy || bidHistory.length===0} style={{ padding:"11px 4px", borderRadius:9, border:"1.5px solid #E2E8F0", background:"#FFFFFF", color:"#64748B", fontSize:12, fontWeight:700, cursor:(busy||bidHistory.length===0)?"not-allowed":"pointer", opacity:bidHistory.length===0?0.5:1 }}>↺ Undo</button>
            <button onClick={doUnsold} disabled={busy} style={{ padding:"11px 4px", borderRadius:9, border:"1.5px solid #EF4444", background:"#FFFFFF", color:"#EF4444", fontSize:12, fontWeight:700, cursor:busy?"not-allowed":"pointer" }}>Unsold</button>
            <button onClick={doSold} disabled={busy || !leadingTeam} style={{ padding:"11px 4px", borderRadius:9, background:"#166534", border:"none", color:"#FFFFFF", fontSize:12, fontWeight:800, cursor:(busy||!leadingTeam)?"not-allowed":"pointer", opacity:!leadingTeam?0.5:1 }}>Sold ✓</button>
          </div>
        </Card>
      )}
      ...
      {registeredCount > 0 && (
        <div style={{ display:"flex", gap:8 }}>
          <select value={jumpTo} onChange={e=>setJumpTo(e.target.value)} style={{ flex:1, padding:"10px 12px", borderRadius:9, border:"1.5px solid #e5e7eb", fontSize:13, outline:"none", background:"#fafafa" }}>
          ...
          <button onClick={doJump} disabled={!jumpTo || busy} style={{ padding:"10px 16px", borderRadius:9, border:"1.5px solid #E2E8F0", background:"#FFFFFF", fontSize:13, fontWeight:700, cursor:(!jumpTo||busy)?"not-allowed":"pointer" }}>Go</button>
        </div>
      )}
    </div>
  )
